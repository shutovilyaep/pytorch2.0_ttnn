...
__attribute__((constructor)) static void init_shim(void) {
    ...
    fprintf(
        stderr,
        "[ttsim_shim] Providing no-op stubs for: libttsim_tensix_reset_deassert, libttsim_tensix_reset_assert\n");
}

__attribute__((destructor)) static void cleanup_shim(void) {
    if (real_libttsim) {
