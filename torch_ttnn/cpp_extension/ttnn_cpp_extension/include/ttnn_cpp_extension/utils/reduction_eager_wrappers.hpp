...
struct reduction_dimlist {
    ...
    [[nodiscard]] static at::Tensor invoke_dimnames(const at::Tensor& a, at::DimnameList dimnames, bool keepdim, c10::optional<at::ScalarType> dtype = c10::nullopt) {
        auto names = a.names();
    ...
    [[nodiscard]] static at::Tensor& invoke_dimnames_into(const at::Tensor& a, at::DimnameList dimnames, bool keepdim, c10::optional<at::ScalarType> dtype, at::Tensor& out) {
        auto names = a.names();
        std::vector<int64_t> positions;
        positions.reserve(dimnames.size());
        for (const auto& dn : dimnames) {
...
struct reduction_dimlist_unbiased_out {
    [[nodiscard]] static at::Tensor& invoke_into(const at::Tensor& in, c10::OptionalArrayRef<int64_t> dim, bool unbiased, bool keepdim, at::Tensor& out) {
        ttnn::Tensor a_tile = tileify(in);
        std::optional<std::variant<int, ttnn::SmallVector<int>>> dim_variant = to_ttnn_dim_variant(dim);
...
struct reduction_dimlist_correction {
    ...
    [[nodiscard]] static at::Tensor invoke(const at::Tensor& a, c10::OptionalArrayRef<int64_t> dim, const c10::optional<c10::Scalar>& correction, bool keepdim) {
        at::Tensor out = make_empty_like_tt(a);
        return invoke_into(a, dim, correction, keepdim, out);
    }
    [[nodiscard]] static at::Tensor& invoke_into(const at::Tensor& in, c10::OptionalArrayRef<int64_t> dim, const c10::optional<c10::Scalar>& correction, bool keepdim, at::Tensor& out) {
    ...
    [[nodiscard]] static at::Tensor invoke_dimnames(const at::Tensor& a, at::DimnameList dimnames, const c10::optional<c10::Scalar>& correction, bool keepdim) {
        // Map names → positions
        auto names = a.names();
        std::vector<int64_t> positions;
        positions.reserve(dimnames.size());
    ...
    [[nodiscard]] static at::Tensor& invoke_dimnames_into(const at::Tensor& a, at::DimnameList dimnames, const c10::optional<c10::Scalar>& correction, bool keepdim, at::Tensor& out) {
        auto names = a.names();
        std::vector<int64_t> positions;
        positions.reserve(dimnames.size());
        for (const auto& dn : dimnames) {
...
struct reduction_dim_pair {
    ...
    [[nodiscard]] static std::tuple<at::Tensor, at::Tensor> invoke_dimnames(const at::Tensor& a, at::DimnameList dimnames, bool keepdim) {
        auto names = a.names();
    ...
    [[nodiscard]] static std::tuple<at::Tensor, at::Tensor> invoke_dimname(const at::Tensor& a, at::Dimname dimname, bool keepdim) {
        auto names = a.names();
        int64_t pos = -1;
        for (int64_t i = 0; i < static_cast<int64_t>(names.size()); ++i) {
    ...
    [[nodiscard]] static std::tuple<at::Tensor&, at::Tensor&> invoke_dimnames_into(const at::Tensor& a, at::DimnameList dimnames, bool keepdim, at::Tensor& values_out, at::Tensor& indices_out) {
        auto names = a.names();
        TORCH_CHECK(dimnames.size() == 1, "names_dim expects a single dimension name");
        int64_t pos = -1;
        for (int64_t i = 0; i < static_cast<int64_t>(names.size()); ++i) {
    ...
    [[nodiscard]] static std::tuple<at::Tensor&, at::Tensor&> invoke_dimname_into(const at::Tensor& a, at::Dimname dimname, bool keepdim, at::Tensor& values_out, at::Tensor& indices_out) {
        auto names = a.names();
        int64_t pos = -1;
        for (int64_t i = 0; i < static_cast<int64_t>(names.size()); ++i) {
            if (names[i] == dimname) { pos = i; break; }
...
struct reduction_dimlist_unbiased {
    [[nodiscard]] static at::Tensor invoke(const at::Tensor& a, c10::OptionalArrayRef<int64_t> dim, bool unbiased, bool keepdim) {
        at::Tensor out = make_empty_like_tt(a);
    ...
    [[nodiscard]] static at::Tensor invoke_dimnames(const at::Tensor& a, at::DimnameList dimnames, bool unbiased, bool keepdim) {
        auto names = a.names();
        std::vector<int64_t> positions;
        positions.reserve(dimnames.size());
        for (const auto& dn : dimnames) {
    ...
    [[nodiscard]] static at::Tensor& invoke_dimnames_into(const at::Tensor& a, at::DimnameList dimnames, bool unbiased, bool keepdim, at::Tensor& out) {
        auto names = a.names();
        std::vector<int64_t> positions;
        positions.reserve(dimnames.size());
        for (const auto& dn : dimnames) {
