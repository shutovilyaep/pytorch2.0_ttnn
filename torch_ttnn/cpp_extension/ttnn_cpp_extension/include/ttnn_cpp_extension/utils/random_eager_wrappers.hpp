...
struct unary_random_seeded {
    ...
    [[nodiscard]] static at::Tensor& invoke_into(const at::Tensor& input, c10::optional<at::Generator> generator, at::Tensor& out) {
...
struct unary_random_uniform {
    [[nodiscard]] static at::Tensor& invoke_inplace(at::Tensor& self, double from, double to, c10::optional<at::Generator> generator = c10::nullopt) {
        return invoke_into(self, from, to, generator, self);
    }
    [[nodiscard]] static at::Tensor& invoke_into(const at::Tensor& input, double from, double to, c10::optional<at::Generator> generator, at::Tensor& out) {
        ttnn::Tensor in_tile = tt_eager::ext::tileify(input);
        static thread_local std::mt19937 rng(std::random_device{}());
        uint32_t seed = generator.has_value() ? static_cast<uint32_t>(generator.value().current_seed()) : rng();
...
struct random_like_rand {
    ...
    static inline std::pair<double, double> int_bounds_default(at::ScalarType st) {
        ...
        return {static_cast<double>(std::numeric_limits<int32_t>::min()), static_cast<double>(static_cast<int64_t>(std::numeric_limits<int32_t>::max()) + 1)};
    }
    static inline ttnn::Tensor sample_f32_like(const ttnn::Tensor& like, double low, double high, uint32_t seed) {
        ...
        return ttnn::rand(shape, *device, ttnn::DataType::FLOAT32, layout, ttnn::DRAM_MEMORY_CONFIG, static_cast<float>(low), static_cast<float>(high), seed);
    }
    ...
    [[nodiscard]] static at::Tensor& invoke_from_inplace(at::Tensor& self, int64_t from, c10::optional<int64_t> to, c10::optional<at::Generator> gen = c10::nullopt) {
        const auto st = self.scalar_type();
    ...
    [[nodiscard]] static at::Tensor& invoke_to_inplace(at::Tensor& self, int64_t to, c10::optional<at::Generator> gen = c10::nullopt) {
        const auto st = self.scalar_type();
