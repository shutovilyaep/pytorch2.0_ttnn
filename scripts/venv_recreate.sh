...
# Ensure ninja is available and set CMAKE_MAKE_PROGRAM to point to system ninja
if command -v ninja >/dev/null 2>&1; then
    export CMAKE_MAKE_PROGRAM="$(which ninja)"
    echo "Using ninja at: $CMAKE_MAKE_PROGRAM"
else
    echo "ERROR: ninja not found. Please install ninja-build package."
    exit 1
fi
# Ensure system PATH includes /usr/bin before venv paths
export PATH="/usr/bin:/usr/local/bin:${PATH}"
rm -rf .cpmcache build build_Release build_Debug ~/.cache/tt-metal-cache /tmp/tt-metal-cache
./build_metal.sh --without-distributed --build-type RelWithDebInfo --build-tests
# ./build_metal.sh
rm -rf python_env
